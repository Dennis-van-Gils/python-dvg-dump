...
from dvg_ringbuffer_cont_c_numpy import RingBuffer as DvG_RingBuffer
...
N = 100
print("RingBuffer strategies")
print("  numpy_ringbuffer           : %.3f ms" %
      (timeit('try1()', setup=setup, number=N)/N*1000))
      
print("  dvg_ringbuffer_cont_c_numpy: %.3f ms" %
      (timeit('try2()', setup=setup, number=N)/N*1000))

print("  deque (slow!)              : %.3f ms" %
